...
TEST_PHP_ARGS="-n -d extension=dom.so -d extension=$PWD/modules/apc.so" \
NO_INTERACTION=1 \
...
: minimal load test
%{_bindir}/php -n -d extension=modules/apc.so -m | grep apc
...
%changelog
* Mon Jan 13 2014 Remi Collet <rcollet@redhat.com> - 3.1.15-0.4.20140912
- upstream has switched from SVN to GIT
- disable shm_strings_buffer in provided configuration
- fix segfault when Zend Opcache is ebabled #1052178

* Tue Sep 24 2013 Remi Collet <rcollet@redhat.com> - 3.1.15-0.3.svn329724
- clean ZTS conditional build
- always run minimal load test
